...
class TypeInfoCacheTestParameters {
  ...
  /*
  This is similar to getPGTypeByOidParams. It's different in that while writing the initial
  characterization tests the name strings returned by getPGType(oid) was different depending on
  whether or not the type had been cached by getPgType(name). Before refactoring, you need to have a
  baseline to know if you're making any undesired behavioral changes. Once it's confirmed that the
  behavior is consistent, this separate parameter set can be removed.
   */
  static Iterable<Object[]> getPGTypeByOidCachedNameParams() {
    Collection<Object[]> cases = new ArrayList<>();
    cases.add(new Object[]{new PgTypeStruct("pg_catalog", "text"), "text", "text[]"});
